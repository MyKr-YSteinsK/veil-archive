...
Final migration-readiness verification on 2026-09-08: `npm ci`, `npm test` (7 files / 38 tests), `npm run build`, `npm audit --omit=optional` (0 vulnerabilities), strict `1.4.0` release coherence, and `git diff --check` passed. The only dependency change was an ordinary non-force lockfile remediation for the Browserslist build-tool chain; `package.json`, product version, runtime code, schema, PWA configuration, and deployment workflow were unchanged. A fresh clone at audit commit `3f3cf8a` had the complete tracked tree, no untracked files, `npm ci --ignore-scripts`/audit/`tsc -b` passed, and the source tests/build plus `/veil-archive/` dev endpoint passed when using the same-version esbuild binary already allowed by this host. The default clean-clone `npm ci` path was blocked twice by this Windows host's application-control policy at newly downloaded `esbuild.exe`; this is an environment-limited gap, not a repository dependency or source failure.
